...
Covers: embedding, vector store, database, LLM, and messaging adapters.
...
from bitmod.interfaces.messaging import OutgoingMessage
...
class TestSlackAdapter:
    ...
    def test_instantiation(self):
        ...
        assert adapter._token == "xoxb-test-token"
    ...
    @patch("bitmod.adapters.msg_slack.httpx.AsyncClient")
    def test_send_message(self, mock_async_client_cls):
        from bitmod.adapters.msg_slack import SlackAdapter
        ...
        msg = OutgoingMessage(channel_id="C123", text="Hello Slack!")
        run_async(adapter.send(msg))
    ...
    @patch("bitmod.adapters.msg_slack.httpx.AsyncClient")
    def test_send_handles_error_gracefully(self, mock_async_client_cls):
        from bitmod.adapters.msg_slack import SlackAdapter
        ...
        adapter = SlackAdapter(token="xoxb-test")
        msg = OutgoingMessage(channel_id="C123", text="fail")
        # Should not raise
        run_async(adapter.send(msg))
    ...
    def test_start_sets_running(self):
        ...
        adapter = SlackAdapter(token="test")
        run_async(adapter.start(AsyncMock()))
...
class TestDiscordAdapter:
    ...
    def test_instantiation(self):
        ...
        assert adapter._token == "discord-token"
    ...
    @patch("bitmod.adapters.msg_discord.httpx.AsyncClient")
    def test_send_message(self, mock_async_client_cls):
        from bitmod.adapters.msg_discord import DiscordAdapter
        ...
        adapter = DiscordAdapter(token="bot-token")
        msg = OutgoingMessage(channel_id="ch123", text="Hello Discord!")
        run_async(adapter.send(msg))

        mock_client.post.assert_called_once()
        call_args = mock_client.post.call_args
        assert "channels/ch123/messages" in call_args[0][0]
    ...
    def test_send_handles_error_gracefully(self, mock_async_client_cls):
        ...
        adapter = DiscordAdapter(token="test")
        run_async(adapter.send(OutgoingMessage(channel_id="ch1", text="fail")))
...
class TestTelegramAdapter:
    ...
    def test_instantiation(self):
        ...
        adapter = TelegramAdapter(token="tg-token")
        assert adapter.platform_name == "telegram"
        assert "tg-token" in adapter._base_url
    ...
    @patch("bitmod.adapters.msg_telegram.httpx.AsyncClient")
    def test_send_message(self, mock_async_client_cls):
        from bitmod.adapters.msg_telegram import TelegramAdapter
        ...
        adapter = TelegramAdapter(token="tg-token")
        msg = OutgoingMessage(channel_id="12345", text="Hello Telegram!")
        run_async(adapter.send(msg))
        ...
        assert call_args[1]["json"]["text"] == "Hello Telegram!"
    ...
    def test_send_handles_error_gracefully(self, mock_async_client_cls):
        ...
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(side_effect=Exception("tg error"))
        mock_async_client_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        mock_async_client_cls.return_value.__aexit__ = AsyncMock(return_value=False)

        adapter = TelegramAdapter(token="test")
        run_async(adapter.send(OutgoingMessage(channel_id="1", text="fail")))
...
class TestWhatsAppAdapter:
    ...
    def test_instantiation(self):
        ...
        adapter = WhatsAppAdapter(token="wa-token", phone_number_id="123456")
        assert adapter.platform_name == "whatsapp"
        assert adapter._token == "wa-token"
        assert adapter._phone_id == "123456"
        assert "123456" in adapter._base_url

    def test_platform_name(self):
        from bitmod.adapters.msg_whatsapp import WhatsAppAdapter

        adapter = WhatsAppAdapter(token="test")
        assert adapter.platform_name == "whatsapp"

    @patch("bitmod.adapters.msg_whatsapp.httpx.AsyncClient")
    def test_send_message(self, mock_async_client_cls):
        from bitmod.adapters.msg_whatsapp import WhatsAppAdapter
        ...
        adapter = WhatsAppAdapter(token="wa-token", phone_number_id="123")
        msg = OutgoingMessage(channel_id="+1234567890", text="Hello WhatsApp!")
        run_async(adapter.send(msg))
    ...
    def test_send_handles_error_gracefully(self, mock_async_client_cls):
        ...
        mock_client = AsyncMock()
        mock_client.post = AsyncMock(side_effect=Exception("wa error"))
        mock_async_client_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        mock_async_client_cls.return_value.__aexit__ = AsyncMock(return_value=False)
...
class TestMatrixAdapter:
    """Tests for MatrixAdapter."""

    def test_instantiation(self):
        from bitmod.adapters.msg_matrix import MatrixAdapter

        adapter = MatrixAdapter(homeserver="https://matrix.example.com", token="mx-token")
        assert adapter.platform_name == "matrix"
        assert adapter._homeserver == "https://matrix.example.com"
        assert adapter._token == "mx-token"

    def test_platform_name(self):
        from bitmod.adapters.msg_matrix import MatrixAdapter

        adapter = MatrixAdapter(token="test")
        assert adapter.platform_name == "matrix"

    @patch("bitmod.adapters.msg_matrix.httpx.AsyncClient")
    def test_send_message(self, mock_async_client_cls):
        from bitmod.adapters.msg_matrix import MatrixAdapter
        ...
        adapter = MatrixAdapter(homeserver="https://matrix.org", token="mx-token")
        msg = OutgoingMessage(channel_id="!room:matrix.org", text="Hello Matrix!")
        run_async(adapter.send(msg))

        mock_client.put.assert_called_once()
        call_args = mock_client.put.call_args
        assert "m.room.message" in call_args[0][0]
    ...
    def test_send_handles_error_gracefully(self, mock_async_client_cls):
        ...
        mock_client = AsyncMock()
        mock_client.put = AsyncMock(side_effect=Exception("matrix error"))
        mock_async_client_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
        mock_async_client_cls.return_value.__aexit__ = AsyncMock(return_value=False)

        adapter = MatrixAdapter(token="test")
        run_async(adapter.send(OutgoingMessage(channel_id="!room:mx", text="fail")))
    ...
    def test_homeserver_trailing_slash_stripped(self):
        ...
        adapter = MatrixAdapter(homeserver="https://matrix.org/", token="test")
